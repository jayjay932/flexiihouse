...
import { SafeTrip, SafeUser } from "@/app/types";
...
const TripDetailsModal: React.FC<TripDetailsModalProps> = ({
  ...
  const { destinations, startDate, endDate, code_reservation, totalPrice, status, transactions } = trip;
  ...
  // Vérifier si les infos détaillées doivent être visibles
  const hasSuccessfulPaidTransaction = transactions?.some(
    (transaction) => transaction.statut === "réussi" && transaction.etat === "payer"
  ) || false;

  const canViewDetails = currentUser?.role === "admin" || 
    (status === "confirmed" && hasSuccessfulPaidTransaction);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center">
      ...
      {/* Modal */}
      <div className="relative w-full h-full sm:h-auto sm:max-h-[90vh] sm:max-w-4xl sm:mx-4 bg-white sm:rounded-xl shadow-2xl flex flex-col animate-in slide-in-from-bottom-4 sm:slide-in-from-bottom-0 sm:zoom-in-95 duration-300">
        ...
        <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-6 py-4 sm:rounded-t-xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-semibold text-gray-900">Détails du voyage</h2>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="relative h-48 sm:h-64">
            ...
            <div className="absolute bottom-4 left-4 right-4 text-white">
              <h3 className="text-xl sm:text-2xl font-bold mb-2">{trip.title}</h3>
          ...
          <div className="p-6 space-y-8">
            ...
            <div className="space-y-4">
              <h4 className="text-lg font-semibold text-gray-900 flex items-center gap-2">
                ...
                Itinéraire de voyage ({destinations.length} {destinations.length > 1 ? 'destinations' : 'destination'})
              </h4>
                  ...
                  <div key={destination.id} className="border border-gray-200 rounded-lg overflow-hidden">
                    <div className="relative h-32 sm:h-40">
                      ...
                      <div className="absolute bottom-3 left-3 right-3 text-white">
                        <h5 className="font-semibold text-lg mb-1">{destination.name}</h5>
                    ...
                    <div className="p-4 bg-gray-50">
                      <div className="grid grid-cols-2 gap-4 mb-3">
                        <div>
                          ...
                          <div className="font-medium text-gray-900">
                            {safeFormatDate(destination.checkIn, "dd MMM yyyy")}
                          </div>
                        </div>
                        <div>
                          <span className="text-xs text-gray-500">Check-out</span>
                          <div className="font-medium text-gray-900">
                            {safeFormatDate(destination.checkOut, "dd MMM yyyy")}
                          </div>
                        </div>
                      </div>
                      ...
                      {destination.accommodation && (
                        <div className="border-t border-gray-200 pt-3">
                          <h6 className="font-medium text-gray-900 mb-2">{destination.accommodation.title}</h6>
                          <div className="flex gap-4 text-sm text-gray-600">
                            <span>{destination.accommodation.guestCount} voyageurs</span>
                            <span>{destination.accommodation.roomCount} chambres</span>
                            <span>{destination.accommodation.bathroomCount} SDB</span>
                          </div>
                          <div className="mt-2 text-sm font-medium text-gray-900">
                            {destination.accommodation.price.toLocaleString()} FCFA/nuit
                          </div>
                        </div>
                      )}
                      ...
                      {destination.activities && destination.activities.length > 0 && (
                        <div className="border-t border-gray-200 pt-3 mt-3">
                          <h6 className="font-medium text-gray-900 mb-2">Activités prévues</h6>
            ...
            {transactions && transactions.length > 0 ? (
              ...
              <div className="space-y-4">
                ...
                <div className="bg-amber-50 border border-amber-200 rounded-lg p-6 text-center">
                  ...
                  <p className="text-sm text-gray-600">
                    Aucune transaction n'a encore été enregistrée pour ce voyage.
                    Les détails complets seront disponibles après validation du paiement.
                  </p>
                </div>
              </div>
            )}
              ...
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <div className="space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900">Code de voyage</p>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        Le statut de votre voyage et de votre paiement est mis à jour automatiquement
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">
                        Contactez notre équipe pour toute question concernant votre voyage
                      </p>
                    </div>
                  </div>
                </div>
