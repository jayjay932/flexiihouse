...
import Modal from './Modal'; // Ton modal custom sans Headless UI
...
const TermsModal: React.FC<TermsModalProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    // ✅ Affiche le modal si l'utilisateur est connecté et n'a pas encore accepté
...
const handleRefuse = async () => {
  try {
    ...
    setIsOpen(false);
    signOut({ callbackUrl: '/' }); // ✅ redirection immédiate vers l’accueil
  } catch {
  ...
  return (
    <Modal isOpen={isOpen} onClose={() => {}} title="Conditions d'utilisation">
      <div className="max-h-[60vh] overflow-y-auto text-sm text-gray-700 px-1 space-y-4">
        <p className="mb-2 font-semibold text-gray-900 text-base">Conditions Générales d’Utilisation</p>
        ...
        <div>
          ...
          <p>Email : <em>support@flexii.com</em><br />
             WhatsApp : <em>+225 XXXXXXXX</em></p>
        </div>
      </div>
